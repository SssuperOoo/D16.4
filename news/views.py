...
class CategoryList(NewsList):
    ...
    def get_queryset(self):
        self.category = get_object_or_404(Category, id=self.kwargs[
            'pk'])  # отличная функция которая будет ловить ошибку, например в случае когда данной категории уже не будет
...
class NewsCreate(PermissionRequiredMixin, LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        post.save()
        # new_post.delay(post.pk) # выключаем таск, чтобы не запускать селери и редис в тестовом режиме.
